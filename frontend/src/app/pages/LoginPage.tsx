...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let creditsMsg = '';
      if (token) {
        const data = await getCredits(token).catch(() => null);
        if (data) creditsMsg = ` You have ${data.credits} credit${data.credits === 1 ? '' : 's'} remaining.`;
      }
      toast.success(`Welcome back!${creditsMsg}`);
      navigate(returnTo);
    } catch (err: any) {
